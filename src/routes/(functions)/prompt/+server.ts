...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	const [countErr, countQuery] = await catchError(
		convexClient.query(api.messages.getAssistantMessageCounts, {
			user: user.id,
	...
	if (thread) {
		const [threadErr, threadQuery] = await catchError(
			convexClient.query(api.threads.getThread, {
				thread
			})
		);
		...
		if (threadQuery.user !== user.id) {
			return json(
				{ success: false, message: 'User does not have access to thread.' },
				{ status: 400 }
			);
		}
		...
		const [historyErr, historyQuery] = await catchError(
			convexClient.query(api.messages.getFinishedMessagesFromThread, {
		...
		const [threadErr, threadMutation] = await catchError(
			convexClient.mutation(api.threads.createUserThread, {
				user: user.id
			})
		);
	...
	if (!modify) {
		const [userErr, userMutation] = await catchError(
			convexClient.mutation(api.messages.createUserMessage, {
		...
		if (userErr) {
			return json(
				{ success: false, message: 'Error trying to create new user message.' },
				{ status: 400 }
			);
		}
	} else {
		const [modifyErr, modifyMutation] = await catchError(
			convexClient.mutation(api.messages.modifyMessage, {
				message: modify,
				newContent: message
			})
		);
	...
	const [assistantErr, assistantMutation] = await catchError(
		convexClient.mutation(api.messages.createUserMessage, {
			threadId: referencedThread,
			role: 'assistant',
			content: '',
	...
	if (assistantErr) {
		...
		return json(
			{ success: false, message: "Assistants message couldn't be created." },
			{ status: 400 }
		);
	}
